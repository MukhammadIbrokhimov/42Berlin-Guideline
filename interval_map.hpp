...
public:
	...
	void assign(K const& keyBegin, K const& keyEnd, V_forward&& val)
		...
		// Step 1: Handle empty interval case
			...
			// Step 2: Find the lower and upper bounds in the existing map
			auto itLow = m_map.lower_bound(keyBegin);  // First key >= keyBegin
			auto itHigh = m_map.lower_bound(keyEnd);   // First key >= keyEnd
			
			// Step 3: Find the value before keyBegin
			V prevValue = (itLow == m_map.begin()) ? m_valBegin : std::prev(itLow)->second;
			
			// Step 4: Avoid unnecessary insertion if the interval already has the same value
			if (prevValue == val) {
				return;
			}
			...
			// Step 7: Restore previous value after keyEnd, if necessary
